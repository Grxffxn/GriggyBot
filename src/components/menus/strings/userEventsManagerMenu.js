...
module.exports = {
  customId: 'adminUserEventsManagerMenu',
  /**
  ...
  run: async (interaction, args) => {
    ...
    if (selectedEvents.length === 0) {
      return interaction.reply({ content: 'No events selected to end.', flags: MessageFlags.Ephemeral });
    }
    selectedEvents.forEach(eventId => {
      endEvent(targetedUserId, eventId);
    });

    const confirmationContainer = new ContainerBuilder().setAccentColor(resolveColor('Green'))
      .addSectionComponents(new SectionBuilder().addTextDisplayComponents([
        new TextDisplayBuilder().setContent(`✅ Ended selected events for <@${targetedUserId}>`),
        new TextDisplayBuilder().setContent(`Ended: ${selectedEvents.join(', ')}`),
