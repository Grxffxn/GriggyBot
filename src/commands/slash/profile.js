...
module.exports = {
  ...
  async run(interaction) {
    ...
    if (!row) {
      ...
      for (const line of linkedAccountsData.split('\n')) {
        const [id, uuid] = line.trim().split(' ');
        if (id && uuid) linkedAccounts[id] = uuid;
      }
      ...
      trimmedUUID = minecraftUUID.replace(/-/g, '');
      ...
      trimmedUUID = row.minecraft_uuid.replace(/-/g, '');
    }
